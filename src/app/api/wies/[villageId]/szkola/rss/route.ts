...
import { zbudujRssTablicySzkoly } from "@/lib/szkola/rss-szkoly";
...
export async function GET(_request: Request, { params }: Props) {
  ...
  const ogloszenia = await pobierzOgloszeniaSzkolyPubliczne(supabase, wies.id, 40);
  const xml = zbudujRssTablicySzkoly({
    nazwaWsi: wies.name,
    villageId: wies.id,
