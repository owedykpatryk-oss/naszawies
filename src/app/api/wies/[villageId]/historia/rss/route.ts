...
import { zbudujRssHistoriiWsi } from "@/lib/historia/rss-historii";
...
export async function GET(_request: Request, { params }: Props) {
  ...
  const wpisy = await pobierzHistoriePublicznaWsi(supabase, wies.id, 40);
  const xml = zbudujRssHistoriiWsi({
    nazwaWsi: wies.name,
    villageId: wies.id,
