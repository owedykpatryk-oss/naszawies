...
type Props = {
  poczatkowe: KluczPanelNawigacji[];
  pokazSoltysa: boolean;
  pokazAdmin: boolean;
};

function opcjeDlaStanu(pokazSoltysa: boolean, pokazAdmin: boolean) {
  return (Object.keys(ETYKIETY_PANEL_NAWIGACJI) as KluczPanelNawigacji[]).filter((k) => {
    const d = ETYKIETY_PANEL_NAWIGACJI[k];
    if (d.wymagaSoltysa && !pokazSoltysa) return false;
    if (d.wymagaAdmina && !pokazAdmin) return false;
    return true;
  });
}

export function ProfilPreferencjePanelKlient({ poczatkowe, pokazSoltysa, pokazAdmin }: Props) {
  const [wybrane, ustawWybrane] = useState<KluczPanelNawigacji[]>(poczatkowe.slice(0, MAX_SLOTOW));
  const [blad, ustawBlad] = useState("");
  const [ok, ustawOk] = useState(false);
  const [czek, startT] = useTransition();

  const dostepne = opcjeDlaStanu(pokazSoltysa, pokazAdmin);

  function przelacz(klucz: KluczPanelNawigacji) {
    ustawWybrane((prev) => {
  ...
  function przesun(klucz: KluczPanelNawigacji, kierunek: -1 | 1) {
    ustawWybrane((prev) => {
  ...
  function zapisz() {
    ...
    ustawOk(false);
    startT(async () => {
      const wynik = await zapiszPreferencjeUi({ panel_nawigacja: wybrane });
      if ("blad" in wynik) {
  ...
  return (
    <section className="panel-karta mt-10 max-w-xl">
      <h2 className="font-serif text-xl text-green-950">Górny pasek panelu</h2>
      <p className="mt-2 text-sm text-stone-600">
        Wybierz od {MIN_SLOTOW} do {MAX_SLOTOW} skrótów w nagłówku panelu (komputer i tablet). Kolejność ustawiasz
        strzałkami.
      </p>
      ...
      <ul className="mt-4 space-y-2">
        {dostepne.map((klucz) => {
          ...
          const def = ETYKIETY_PANEL_NAWIGACJI[klucz];
      ...
      {ok ? <p className="mt-2 text-sm text-green-800">Zapisano — odśwież stronę panelu, jeśli pasek się nie zmienił.</p> : null}
      ...
      >
        {czek ? "Zapisywanie…" : "Zapisz pasek panelu"}
      </button>
    </section>
  );
}
