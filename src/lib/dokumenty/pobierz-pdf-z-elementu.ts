...
/** Zwraca PDF jako base64 (do załącznika e-mail). */
export async function generujPdfBase64ZElementuHtml(
  zrodlo: HTMLElement,
): Promise<{ ok: true; base64: string; nazwa: string } | { ok: false; komunikat: string }> {
  if (typeof document === "undefined") {
    return { ok: false, komunikat: "Funkcja dostępna tylko w przeglądarce." };
  }
  ...
  try {
    ...
    const worker = html2pdf().from(wrap).set({
      margin: [8, 8, 8, 8],
      image: { type: "jpeg", quality: 0.93 },
      html2canvas: {
        scale,
    ...
    const base64 = await new Promise<string>((resolve, reject) => {
      ...
      reader.onload = () => {
        const wynik = reader.result as string;
        const czysty = wynik.includes(",") ? wynik.split(",")[1]! : wynik;
        resolve(czysty);
      };
    ...
    return { ok: true, base64, nazwa: "zaproszenie.pdf" };
  } catch (e) {
    const komunikat = e instanceof Error ? e.message : "Nie udało się utworzyć pliku PDF.";
    return { ok: false, komunikat };
  } finally {
