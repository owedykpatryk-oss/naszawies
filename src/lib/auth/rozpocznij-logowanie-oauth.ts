...
export type DostawcaOAuthPubliczny = "google" | "facebook";

const ETYKIETY_DOSTAWCY: Record<DostawcaOAuthPubliczny, string> = {
  google: "Google",
  facebook: "Facebook",
};
...
/** Wspólna logika OAuth (Google, Facebook) — Turnstile, rate limit, URL przekierowania. */
export async function rozpocznijLogowanieOAuth(
  request: NextRequest,
  provider: DostawcaOAuthPubliczny,
): Promise<NextResponse> {
  const etykieta = ETYKIETY_DOSTAWCY[provider];
  ...
  if (error || !data.url) {
    ...
    return NextResponse.json(
      {
        error: nieWlaczony
          ? `Logowanie przez ${etykieta} nie jest jeszcze skonfigurowane. Skontaktuj się z administratorem.`
          : error?.message || `Nie udało się rozpocząć logowania przez ${etykieta}.`,
      },
      { status: 400 },
    );
  }
