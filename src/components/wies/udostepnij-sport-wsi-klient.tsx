...
export function UdostepnijSportWsiKlient({ sciezkaProfilu, nazwaWsi }: Props) {
  const [kopiuj, ustawKopiuj] = useState<"idle" | "ok" | "blad">("idle");
  const link =
    typeof window !== "undefined"
      ? `${window.location.origin}${sciezkaProfilu}#sekcja-sport`
      : `${sciezkaProfilu}#sekcja-sport`;
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({
          title: `Terminarz sportowy — ${nazwaWsi}`,
          text: "Mecze i treningi na naszawies.pl",
          url: link,
        });
        return;
      } catch {
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-2">
      <button
        ...
        className={`${przycisk} border-emerald-500 bg-emerald-800 text-white shadow-sm hover:bg-emerald-900`}
      >
        Udostępnij terminarz
      </button>
      <button
        type="button"
