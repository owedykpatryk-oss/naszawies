-- Publiczny podgląd sal świetlicy na profilu wsi (bez ujawniania asortymentu / RLS member-only)

CREATE OR REPLACE FUNCTION public.halls_publiczne_dla_wsi(p_village_id uuid)
RETURNS TABLE (
  id uuid,
  name text,
  address text,
