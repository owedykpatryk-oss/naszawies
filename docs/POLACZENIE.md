...
8b. **E-mail po rejestracji (potwierdzenie)** — wysyła go **Supabase Auth**, nie formularz kontaktowy (Resend). **Wygląd maili:** HTML w `supabase/templates/email/` — wgranie na hosting: `SUPABASE_ACCESS_TOKEN` w `.env.local` (token z [Account → Access Tokens](https://supabase.com/dashboard/account/tokens)), potem `npm run wgraj:szablony-maili` (szczegóły: `docs/SZABLONY_MAILI_SUPABASE.md`). Jeśli nikt nie dostaje maila:
...
| **Pełny katalog miejscowości (nazwy z SIMC, cała Polska)** — pod wyszukiwarkę i wniosek sołtysa | Pobierz `TERC.xml` + `SIMC.xml` (lub warianty urzędowe) z [GUS eTeryt](https://eteryt.stat.gov.pl/eTeryt/rejestr_teryt/udostepnianie_danych/baza_teryt/udostepnianie_danych/baza_teryt.aspx?contrast=default), potem: `npm run import-teryt -- --dry-run "docs/Pliki/TERC_Urzedowy_2026-04-25.xml" "docs/Pliki/SIMC_Urzedowy_2026-04-25.xml"` (dostosuj datę / nazwę) lub zapis: `npm run import-teryt -- …` + `SUPABASE_SERVICE_ROLE_KEY` w `.env.local`. Import domyślnie bierze **wszystkie** wiersze miejscowości z SIMC; węższy tryb: `npm run import-teryt -- --zawez-do-rm …`. Rekordy mają `is_active = false` do czasu objęcia wsi przez aktywnego sołtysa (logika w triggerze). |
